const translations = {
  id: {
    // Navbar
    "nav.home": "Beranda",
    "nav.products": "Produk",
    "nav.about": "Tentang",
    "nav.elkkp": "INAPROC",
    "nav.contact": "Kontak",
    "nav.portfolio": "Portofolio",

    // Hero Section
    "hero.badge": "Pemimpin Industri Sejak 2004",
    "hero.title": "Supplier Penggilingan Padi: ",
    "hero.titleHighlight": "Solusi Presisi",
    "hero.titleSuffix": " untuk Hasil Beras Premium",
    "hero.subtitle": "Lebih dari 20 tahun memimpin manufaktur dan instalasi Rice Milling Unit (RMU) berstandar nasional. Kami hadirkan teknologi efisien untuk ketahanan pangan Indonesia.",
    "hero.cta1": "Konsultasi Rencana Usaha",
    "hero.cta2": "Lihat Katalog INAPROC",
    "hero.ctaWa": "Hubungi Langsung Kami",
    "hero.stat": "Tahun Pengalaman",

    // TrustBar
    "trustBar.label": "Didukung oleh Mitra Strategis & Bersertifikasi Nasional",

    // ValueProp
    "valueProp.title1": "Tenaga Spesialis Ahli",
    "valueProp.desc1": "Setiap unit dirakit oleh teknisi berpengalaman dengan kontrol kualitas yang ketat untuk memastikan presisi operasional.",
    "valueProp.title2": "Pelatihan Operator Gratis",
    "valueProp.desc2": "Kami memberikan modul pelatihan intensif agar tim Anda dapat mengoperasikan mesin dengan efisiensi maksimal sejak hari pertama.",
    "valueProp.title3": "Garansi & Purnajual",
    "valueProp.desc3": "Dukungan ketersediaan suku cadang asli dan layanan purnajual responsif di seluruh wilayah operasional Anda.",

    // ProductGrid
    "productGrid.title": "Solusi Rice Milling Unit",
    "productGrid.subtitle": "Teknologi fabrikasi presisi untuk berbagai skala usaha penggilingan padi, dari kapasitas menengah hingga industri ekspor.",
    "productGrid.exploreAll": "Lihat Semua Produk",
    "productGrid.detailSpec": "Detail Spesifikasi",
    "productGrid.specLabel": "Spesifikasi Unit/Paket:",
    "productGrid.close": "Tutup",
    "productGrid.consult": "Konsultasi Paket",

    // Portfolio (component)
    "portfolio.title": "Instalasi & Proyek Kami",
    "portfolio.subtitle": "Lebih dari 20 tahun menghadirkan rekayasa presisi dan Rice Milling Unit handal di seluruh Indonesia.",
    "portfolio.viewMore": "Lihat Lebih Banyak",

    // ContactMe / Contact Home
    "contact.title": "Hubungi Kami",
    "contact.phone": "Telepon / WhatsApp",
    "contact.whatsapp": "WhatsApp",
    "contact.call": "Telepon",
    "contact.badge": "Ateka Tehnik Hub",
    "contact.subtitle": "Kunjungi bengkel dan kantor operasional kami atau hubungi tim representatif kami untuk konsultasi produk yang Anda butuhkan.",
    "contact.addressLabel": "Alamat Bengkel Utama",
    "contact.addressValue": "Jl. Grompol - Jambangan, Gondang, Kedungjeruk, Kec. Mojogedang, Kabupaten Karanganyar, Jawa Tengah",
    "contact.mapsLabel": "Lokasi Karanganyar",

    // OfficialChannelNotice
    "notice.title": "Peringatan Keamanan",
    "notice.descPre": "Website dengan domain ",
    "notice.fakeDomain": "atekateknik.com",
    "notice.descMid": " (menggunakan huruf 'k') adalah ",
    "notice.notOfficial": "BUKAN saluran resmi kami",
    "notice.descPost": ". Segala penawaran maupun transaksi di luar kontak resmi bukan tanggung jawab kami.",
    "notice.locationLabel": "Satu-satunya Lokasi Resmi",
    "notice.locationDesc": "Kantor & Bengkel operasional kami HANYA berlokasi di ",
    "notice.locationCity": "Karanganyar, Jawa Tengah",
    "notice.fakeLabel": "Contoh Channel Tidak Resmi",
    "notice.fakeDesc": "Website dan alamat di peta dengan bentuk menyerupai latar di belakang ini adalah ",
    "notice.fakeDescPost": " milik Ateka Tehnik. Jangan mudah percaya jika dihubungi atas nama kami dari selain nomor dan alamat resmi.",
    "notice.hoverLabel": "Tangkapan Layar Tidak Resmi",
    "notice.ctaBtn": "Lihat Daftar Channel Resmi Kami",

    // ChatbotWidget
    "chatbot.name": "Ateka AI Assistant",
    "chatbot.status": "Online & Siap Membantu",
    "chatbot.welcome": "Halo! Selamat datang di CV Ateka Tehnik. Ada yang bisa saya bantu terkait kebutuhan manufaktur mesin penggilingan padi (RMU) untuk bisnis Anda hari ini?",
    "chatbot.popularLabel": "Pertanyaan Populer:",
    "chatbot.q1": "Apa spesifikasi RMU Paket Menengah?",
    "chatbot.q2": "Bagaimana prosedur instalasi unit?",
    "chatbot.q3": "Apakah melayani pelatihan operator?",
    "chatbot.q4": "Berapa estimasi biaya Paket Premium?",
    "chatbot.placeholder": "Ketik balasan Anda...",
    "chatbot.fullscreenExit": "Keluar layar penuh",
    "chatbot.fullscreenEnter": "Layar penuh",
    "chatbot.closeChat": "Tutup chat",
    "chatbot.contactAgent": "Hubungi Agen",
    "chatbot.techInquiry": "Permintaan Teknis",
    "chatbot.disclaimer": "Respons AI mungkin tidak selalu akurat",

    // Footer
    "footer.desc": "Pionir teknologi pengolahan hasil bumi dengan fokus pada kualitas dan keberlanjutan sektor pertanian Indonesia.",
    "footer.navigation": "Navigasi",
    "footer.home": "Beranda",
    "footer.products": "Produk",
    "footer.projects": "Proyek",
    "footer.quickLinks": "Tautan Cepat",
    "footer.privacy": "Kebijakan Privasi",
    "footer.terms": "Syarat Layanan",
    "footer.faq": "Tanya Jawab (FAQ)",
    "footer.globalExport": "Panduan & Edukasi",
    "footer.contactOffice": "Kantor Pusat",
    "footer.copyright": "© 2026 CV Ateka Tehnik - Hak Cipta Dilindungi.",
    "footer.premiumQuality": "Kualitas Premium",

    // About Page
    "about.badge": "Est. 2004",
    "about.heroTitle1": "Merekayasa Masa Depan ",
    "about.heroHighlight": "Penggilingan Padi",
    "about.heroSub": "Dua dekade penguasaan teknis dalam rekayasa pertanian, didedikasikan untuk ketahanan pangan Indonesia dan keunggulan industri.",
    "about.storyTitle": "Warisan Presisi Industri",
    "about.storyP1": "Didirikan pada tahun 2004, CV Ateka Tehnik bermula dari satu visi: menjembatani kesenjangan antara praktik pertanian tradisional dan efisiensi industri modern. Selama lebih dari ",
    "about.storyP1Bold": "20 tahun",
    "about.storyP1End": ", kami telah berevolusi dari bengkel lokal menjadi pemimpin nasional yang diakui dalam teknologi penggilingan padi.",
    "about.storyP2Start": "Sebagai ",
    "about.storyP2Bold": "vendor resmi LKPP",
    "about.storyP2End": ", kami bangga berkontribusi langsung pada inisiatif ketahanan pangan nasional. Mesin-mesin kami bukan sekadar peralatan; mereka adalah jantung operasi penggilingan di seluruh nusantara, direkayasa untuk bertahan dalam kerasnya iklim dan kebutuhan throughput Indonesia.",
    "about.statYears": "Tahun Pengalaman",
    "about.statInstallations": "Instalasi",
    "about.statProvinces": "Provinsi Terlayani",
    "about.quote": "\"Presisi adalah standar dasar kami. Kemajuan adalah misi kami.\"",
    "about.quoteAuthor": "Warsito - Direktur Pendiri",
    "about.coreTitle": "Prinsip Inti Kami",
    "about.precisionTitle": "Presisi Tak Tertandingi",
    "about.precisionDesc": "Kami menerapkan toleransi tingkat dirgantara dalam perakitan penggilingan kami untuk memastikan kerusakan butir minimum dan konsistensi hasil maksimum.",
    "about.innovationTitle": "Inovasi Berkelanjutan",
    "about.innovationDesc": "Tim R&D kami mengintegrasikan pemantauan IoT dan otomasi ke setiap mesin, membawa revolusi Industri 4.0 ke sawah.",
    "about.partnershipTitle": "Kemitraan Strategis",
    "about.partnershipDesc": "Kami tidak sekadar menjual mesin; kami membangun aliansi seumur hidup dengan para penggilingan melalui dukungan on-site dan pelatihan pemeliharaan.",
    "about.leadershipTitle": "Kepemimpinan Rekayasa",
    "about.leadershipSub": "Dipandu oleh tim spesialis mekanik dan visioner industri dengan pengalaman lapangan bertahun-tahun.",
    "about.certLabel": "Standar & Kepatuhan",
    "about.certTitle": "Keunggulan Bersertifikasi",
    "about.ctaTitle": "Siap mengoptimalkan kapasitas penggilingan Anda?",
    "about.ctaBtn1": "Diskusikan Proyek Anda",
    "about.ctaBtn2": "Kunjungi Toko Shopee Kami",

    // Products Page
    "products.badge": "Rekayasa Presisi",
    "products.heroTitle": "Solusi Rekayasa Kami",
    "products.heroSub": "Rasakan konvergensi stabilitas struktural dan inovasi pertanian. Setiap RMU adalah mahakarya baja yang diproduksi presisi.",
    "products.catAll": "Semua Produk",
    "products.catPaketLengkap": "Paket Lengkap RMU",
    "products.catUnitMesin": "Unit Mesin Tunggal",
    "products.catPeralatan": "Peralatan Pendukung",
    "products.catSukuCadang": "Suku Cadang",
    "products.inquire": "Tanyakan Sekarang",
    "products.buyOnShopee": "Beli di Shopee",
    "products.loading": "Memuat produk...",
    "products.noProducts": "Belum ada produk.",
    "products.downloadSpec": "Unduh Spesifikasi",
    "products.compareTitle": "Perbandingan Paket",
    "products.thFeature": "Fitur Teknis",
    "products.thStandard": "Paket Standar",
    "products.thBusiness": "Bisnis Pro",
    "products.thPremium": "Premium Enterprise",
    "products.spareParts": "Suku Cadang Asli",
    "products.spareDesc": "Setiap gear dan belt direkayasa dengan standar ketat yang sama dengan unit utama kami. Pastikan keawetan dengan Ateka Original.",
    "products.catalogDownload": "Unduh Katalog",
    "products.visitOfficialStore": "Kunjungi Toko Resmi Kami",
    "products.support247": "Dukungan Teknis 24/7",
    "products.supportDesc": "Teknisi kami siap membantu Anda di mana saja di nusantara, memastikan waktu henti minimum untuk produksi Anda.",
    "products.isoCertified": "Bersertifikasi ISO",
    "products.isoDesc": "Standar rekayasa global diterapkan secara lokal.",
    "products.globalExport": "Ekspor Global",
    "products.globalDesc": "Melayani Asia Tenggara, Afrika, dan seterusnya.",
    "productDetail.engineeringIntro": "Selama lebih dari dua dekade, Ateka Tehnik telah berada di garis depan rekayasa pertanian di Indonesia. Produk {product} mewakili komitmen kami untuk menjembatani kesenjangan antara pertanian skala kecil dan pemrosesan industri raksasa.",

    // Contact Page
    "contactPage.heroTitle": "Mulai Konsultasi Industri Anda",
    "contactPage.heroSub": "Direkayasa untuk presisi. Dibangun untuk skala besar. Hubungi spesialis teknik kami untuk mengoptimalkan infrastruktur penggilingan padi Anda.",
    "contactPage.formTitle": "Permintaan Teknis",
    "contactPage.labelName": "Nama Lengkap",
    "contactPage.labelCompany": "Nama Perusahaan",
    "contactPage.labelCapacity": "Kapasitas RMU (Ton/Jam)",
    "contactPage.selectCapacity": "Pilih Kapasitas",
    "contactPage.labelLocation": "Lokasi",
    "contactPage.placeholderLocation": "Kota, Provinsi",
    "contactPage.labelEmail": "Alamat Email",
    "contactPage.labelPhone": "Nomor Telepon / WA",
    "contactPage.labelMessage": "Pesan Detail",
    "contactPage.messagePlaceholder": "Jelaskan kebutuhan teknis Anda...",
    "contactPage.submitBtn": "Kirim Permintaan",
    "contactPage.hqLabel": "Kantor Pusat",
    "contactPage.hqTitle": "Pabrik & Kantor Pusat",
    "contactPage.waSupport": "Dukungan WhatsApp",
    "contactPage.emailLabel": "Korespondensi Email",
    "contactPage.exportTitle": "Kapabilitas Ekspor",
    "contactPage.exportDesc": "Kami menawarkan pengiriman global dan layanan instalasi untuk sistem penggilingan kami. Tim kami mengkhususkan diri dalam logistik industri lintas negara.",
    "contactPage.visitTitle": "Kunjungan Pabrik",
    "contactPage.visitDesc": "Kunjungan hanya dengan perjanjian. Silakan hubungi tim dukungan kami setidaknya 48 jam sebelumnya.",
    "contactPage.successTitle": "Permintaan Terkirim!",
    "contactPage.successDesc": "Terima kasih, pesan Anda telah kami terima. Tim kami akan segera menghubungi Anda.",
    "contactPage.errorTitle": "Gagal Mengirim",
    "contactPage.errorDesc": "Maaf, terjadi kesalahan saat mengirim pesan Anda. Silakan coba lagi atau hubungi kami via WhatsApp.",

    // Portfolio Page
    "portfolioPage.heroTitle": "Instalasi &\nProyek Kami",
    "portfolioPage.heroSub": "Lebih dari 20 tahun menghadirkan Rice Milling Unit presisi di seluruh nusantara Indonesia.",
    "portfolioPage.featuredLabel": "Studi Kasus Unggulan",
    "portfolioPage.featuredTitle": "Pemasangan RMU 2–3 Ton/Jam",
    "portfolioPage.featuredDesc": "Proyek pemasangan Rice Milling Unit berkapasitas 2–3 ton per jam di Palembang, Sumatera Selatan. Unit ini dirancang untuk memenuhi kebutuhan penggilingan padi skala Industri dengan efisiensi tinggi, menghasilkan beras berkualitas premium siap distribusi.",
    "portfolioPage.featuredCapacityValue": "2–3 Ton/Jam",
    "portfolioPage.statCapacity": "Kapasitas Operasional",
    "portfolioPage.statPurity": "Rating Kemurnian",
    "portfolioPage.viewSpecs": "Lihat spesifikasi teknis",
    "portfolioPage.gridLabel": "Portofolio Proyek",
    "portfolioPage.gridTitle": "Keunggulan Rekayasa Nasional",
    "portfolioPage.presenceLabel": "Kehadiran Kami",
    "portfolioPage.presenceTitle": "Melayani 34 Provinsi",
    "portfolioPage.presenceDesc": "Dari ujung barat Sumatera hingga lanskap timur Papua, jejak rekayasa Ateka Tehnik membentang di seluruh kepulauan Indonesia. Tim teknisi mobile kami memastikan bahwa sekecil apapun fasilitas Anda, perawatan presisi selalu dalam jangkauan.",
    "portfolioPage.statInstall": "500+ Instalasi",
    "portfolioPage.statInstallSub": "Beroperasi secara nasional",
    "portfolioPage.statSupport": "Dukungan Teknis 24/7",
    "portfolioPage.statSupportSub": "Di 3 zona waktu",
    "portfolioPage.ctaTitle": "Siap Merekayasa Masa Depan Anda?",
    "portfolioPage.ctaDesc": "Baik Anda ingin meningkatkan fasilitas yang sudah ada atau memulai usaha penggilingan baru, tim teknisi kami siap merancang jalur presisi Anda.",
    "portfolioPage.ctaBtn1": "Mulai Konsultasi Proyek",
    "portfolioPage.ctaBtn2": "Lihat Profil Perusahaan",

    // Post Page
    "postPage.notFound": "Post Tidak Ditemukan",
    "postPage.notFoundDesc": "Artikel yang diminta tidak dapat ditemukan.",
    "postPage.backToPortfolio": "Kembali ke Portofolio",
    "postPage.noCoverImage": "Tidak Ada Gambar Sampul",
    "postPage.projectOverview": "Ikhtisar Proyek",
    "postPage.share": "Bagikan",
    "postPage.keyDeliverables": "Deliverable Utama",
    "postPage.techSpecs": "Spesifikasi Teknis",
    "postPage.viewEquipment": "Lihat Peralatan",
    "postPage.mediaGallery": "Galeri Media",
    "postPage.comments": "Komentar",
    "postPage.addComment": "Tambahkan komentar...",
    "postPage.postComment": "Kirim Komentar",
    "postPage.noComments": "Belum ada komentar. Jadilah yang pertama berbagi pendapat!",
    "postPage.otherInstallations": "Instalasi Terbaru Lainnya",
    "postPage.viewAllProjects": "Lihat semua proyek",
    "postPage.readCaseStudy": "Baca Studi Kasus",
    "postPage.ctaTitle": "Siap Mengoptimalkan\nHasil Anda?",
    "postPage.ctaDesc": "Ateka Tehnik menyediakan solusi rekayasa menyeluruh untuk industri pertanian. Dari desain hingga komisioning, kami menghadirkan presisi.",
    "postPage.ctaBtn": "Konsultasikan Proyek Anda",
    "postPage.confirmComment": "Konfirmasi Komentar Anda",
    "postPage.commentReview": "Komentar Anda akan ditinjau sebelum ditampilkan secara publik.",
    "postPage.postAsAnonymous": "Kirim sebagai Anonim",
    "postPage.yourName": "Nama Anda",
    "postPage.enterName": "Masukkan nama Anda...",
    "postPage.cancel": "Batal",
    "postPage.submit": "Kirim",
    "postPage.sharePost": "Bagikan postingan ini",
    "postPage.copyLink": "Salin Tautan",
    "postPage.copied": "Tersalin!",

    // Official Channels Page
    "channels.badge": "Verifikasi Channel",
    "channels.title": "Daftar Channel Resmi ",
    "channels.titleHighlight": "Ateka Tehnik",
    "channels.subtitle": "Pastikan Anda hanya bertransaksi dan menghubungi kami melalui jalur komunikasi dan lokasi yang tertera di bawah ini.",
    "channels.officialTitle": "Channel RESMI Kami",
    "channels.officialSub": "Valid & Terverifikasi (Aman)",
    "channels.officialMapLabel": "Lokasi Pabrik Induk (Karanganyar)",
    "channels.openMaps": "Buka di Google Maps",
    "channels.socialLabel": "Media Sosial Resmi",
    "channels.fakeTitle": "Channel PALSU / BUKAN Resmi",
    "channels.fakeSub": "Harap Hindari Bertransaksi Disini!",
    "channels.fakeMapLabel": "Lokasi Modus (Tangerang) - BUKAN KAMI",
    "channels.ignoreLocation": "Abaikan lokasi ini",
    "channels.viewRef": "Lihat referensi modus",
    "channels.fakeWebLabel": "Website Tiruan Merek Kami",
    "channels.fakeWebNote": "Menggunakan huruf 'k' - BUKAN KAMI",
    "channels.fakeWebVisit": "Kunjungi",
    "channels.backHome": "Kembali ke Beranda",

    // Privacy Policy
    "privacy.title": "Kebijakan Privasi",
    "privacy.lastUpdated": "Terakhir diperbarui: 2 April 2026",
    "privacy.intro": "CV Ateka Tehnik (\"kami\", \"kita\", atau \"milik kami\") mengoperasikan situs web atekatehnik.com dan layanan terkait (\"Layanan\"). Halaman ini memberi tahu Anda tentang kebijakan kami terkait pengumpulan, penggunaan, dan pengungkapan data pribadi saat Anda menggunakan Layanan kami serta pilihan yang telah Anda kaitkan dengan data tersebut.",
    "privacy.infoCollectionLabel": "1. Pengumpulan dan Penggunaan Informasi",
    "privacy.infoCollectionDesc": "Kami mengumpulkan beberapa jenis informasi untuk berbagai tujuan dalam menyediakan dan meningkatkan Layanan kami kepada Anda. Informasi ini dapat mencakup Nama, Alamat Email, Nomor Telepon (termasuk WhatsApp), Alamat Pengiriman (seperti untuk pemesanan), dan Data Penggunaan Situs.",
    "privacy.useDataLabel": "2. Penggunaan Data",
    "privacy.useDataDesc": "CV Ateka Tehnik menggunakan data yang dikumpulkan untuk berbagai tujuan: menyediakan dan memelihara Layanan, memberi tahu Anda tentang perubahan Layanan, memberikan dukungan dan konsultasi teknis pelanggan (seperti pertanyaan seputar RMU), memfasilitasi transaksi melalui LKPP atau e-katalog, dan memantau penggunaan Layanan.",
    "privacy.transferDataLabel": "3. Transfer Data",
    "privacy.transferDataDesc": "Sistem informasi kami berpusat di Indonesia. Dengan menggunakan layanan kami, Anda menyetujui pemrosesan data di Indonesia sesuai dengan regulasi setempat. Kami akan mengambil langkah wajar untuk memastikan bahwa data Anda diperlakukan secara aman.",
    "privacy.disclosureLabel": "4. Pengungkapan Data",
    "privacy.disclosureDesc": "Kami dapat mengungkapkan Data Pribadi Anda dengan itikad baik bahwa tindakan tersebut diperlukan untuk: mematuhi kewajiban hukum, melindungi dan membela hak atau properti CV Ateka Tehnik, atau mencegah/menyelidiki kesalahan terkait Layanan.",
    "privacy.securityLabel": "5. Keamanan Data",
    "privacy.securityDesc": "Keamanan data Anda penting bagi kami. Meskipun tidak ada metode transmisi di Internet yang 100% aman, kami berupaya menggunakan cara-cara yang diterima secara komersial untuk melindungi Data Pribadi Anda.",
    "privacy.contactLabel": "6. Hubungi Kami",
    "privacy.contactDesc": "Jika Anda memiliki pertanyaan tentang Kebijakan Privasi ini, silakan hubungi kami via WhatsApp atau di kantor pusat kami di Dusun Gondang, RT 02 RW 01, Desa Kedungjeruk, Kec. Mojogedang, Kabupaten Karanganyar, Jawa Tengah.",

    // Terms of Service
    "terms.title": "Syarat Layanan",
    "terms.lastUpdated": "Terakhir diperbarui: 2 April 2026",
    "terms.intro": "Harap baca Syarat Layanan (\"Syarat\", \"Syarat Layanan\") ini dengan cermat sebelum menggunakan situs web atekatehnik.com (\"Layanan\") yang dioperasikan oleh CV Ateka Tehnik.",
    "terms.acceptanceLabel": "1. Penerimaan Syarat",
    "terms.acceptanceDesc": "Akses Anda ke dan penggunaan Layanan kami dikondisikan pada penerimaan dan kepatuhan Anda terhadap Syarat ini. Syarat ini berlaku untuk semua pengunjung, pengguna, dan orang lain yang mengakses Layanan.",
    "terms.servicesLabel": "2. Layanan Konsultasi & Penjualan",
    "terms.servicesDesc": "CV Ateka Tehnik menyediakan jasa manufaktur, instalasi, dan konsultasi mesin Rice Milling Unit (RMU). Informasi di situs ini merangkum spesifikasi standar; spesifikasi akhir setiap proyek dapat disesuaikan (custom) dan disepakati melalui kontrak atau proses E-Katalog LKPP secara terpisah.",
    "terms.intellectualLabel": "3. Hak Kekayaan Intelektual",
    "terms.intellectualDesc": "Layanan dan konten asli, fitur, serta fungsionalitasnya (seperti desain antarmuka, gambar mesin Ateka, dokumen spesifikasi teknis) adalah milik eksklusif dari CV Ateka Tehnik. Konten tersebut tidak boleh direproduksi tanpa izin tertulis.",
    "terms.linksLabel": "4. Tautan ke Situs Web Lain",
    "terms.linksDesc": "Layanan kami mungkin berisi tautan ke situs web pihak ketiga yang tidak dimiliki atau dikontrol oleh CV Ateka Tehnik (misalnya, platform Indotrading atau LKPP). Kami tidak bertanggung jawab atas konten atau praktik privasi situs pihak ketiga mana pun.",
    "terms.liabilityLabel": "5. Batasan Kewajiban",
    "terms.liabilityDesc": "Dalam keadaan apa pun CV Ateka Tehnik tidak bertanggung jawab atas segala kerugian tidak langsung, insidental, atau konsekuensial yang timbul dari akses atau penggunaan materi pada Layanan web ini. Untuk jaminan produk fisik seperti RMU, hal tersebut dicakup dalam Syarat Garansi terpisah.",
    "terms.governingLabel": "6. Hukum yang Berlaku",
    "terms.governingDesc": "Syarat ini akan diatur dan ditafsirkan sesuai dengan hukum Republik Indonesia. Segala perselisihan yang mungkin timbul akan diselesaikan di bawah yurisdiksi pengadilan di Indonesia yang sesuai.",

    // Edukasi Highlight (Home CTA)
    "edukasi.highlightTitle": "Mengapa Anda Butuh Mesin Modern?",
    "edukasi.highlightDesc": "Ketahuilah rahasia cara kerja mesin modern Rice Milling Unit (RMU) dan Bed Dryer dalam mengatasi masalah susut hasil, kualitas patah, hingga menghemat biaya operasional Anda.",
    "edukasi.highlightCta": "Pelajari Selengkapnya",

    // Edukasi Page
    "edukasi.badge": "Pusat Pengetahuan",
    "edukasi.heroTitle": "Edukasi Teknologi ",
    "edukasi.heroHighlight": "Pasca Panen",
    "edukasi.heroDesc": "Pahami lebih dalam tentang proses penggilingan dan pengeringan modern untuk menghasilkan beras dengan kualitas super dan meminimalisir susut hasil pertanian.",
    "edukasi.ctaTitle": "Siap Meningkatkan Produksi Anda?",
    "edukasi.ctaDesc": "Ambil langkah selanjutnya untuk memajukan usaha penggilingan padi Anda dengan inovasi modern.",
    "edukasi.ctaHome": "Kembali ke Beranda",
    "edukasi.ctaTech": "Permintaan Teknis",
    "edukasi.ctaWa": "Hubungi via WhatsApp",

    // RMU Workflow Section
    "rmuWorkflow.title": "Cara Kerja Mesin RMU",
    "rmuWorkflow.desc": "Proses otomatisasi terpadu untuk mengubah gabah mentah menjadi beras kualitas premium siap konsumsi.",
    "rmuWorkflow.step1Title": "Pembersihan Awal (Pre-Cleaning)",
    "rmuWorkflow.step1Desc": "Pembersihan gabah dari kotoran, batu, dan benda asing lainnya.",
    "rmuWorkflow.step2Title": "Pengupasan Kulit (Husking)",
    "rmuWorkflow.step2Desc": "Memisahkan kulit gabah (sekam) dari butiran beras pecah kulit.",
    "rmuWorkflow.step3Title": "Pemilahan Sekam & Beras",
    "rmuWorkflow.step3Desc": "Pemisahan sempurna antara sekam, beras pecah kulit, dan gabah.",
    "rmuWorkflow.step4Title": "Penyosohan (Polishing)",
    "rmuWorkflow.step4Desc": "Menghilangkan lapisan bekatul untuk menghasilkan beras putih mengkilap.",
    "rmuWorkflow.step5Title": "Pemisahan Beras (Grading)",
    "rmuWorkflow.step5Desc": "Memisahkan beras kepala, beras patah, dan menir sesuai standar.",
    "rmuWorkflow.step6Title": "Pengemasan (Packing)",
    "rmuWorkflow.step6Desc": "Beras siap dikemas ke dalam karung dengan berat yang akurat.",

    // RMU Advantages Section
    "rmuAdv.title": "Keunggulan Mesin RMU",
    "rmuAdv.desc": "Investasi cerdas untuk penggilingan padi modern. Kami menghadirkan nilai tambah yang signifikan pada setiap butir beras yang Anda produksi.",
    "rmuAdv.efficiency": "Efisiensi Proses",
    "rmuAdv.efficiencyDesc": "Hemat waktu, tenaga, dan biaya operasional harian.",
    "rmuAdv.quality": "Beras Berkualitas",
    "rmuAdv.qualityDesc": "Beras lebih bersih, putih, utuh, dan mengkilap.",
    "rmuAdv.productivity": "Produktivitas Tinggi",
    "rmuAdv.productivityDesc": "Kapasitas pengolahan besar dalam waktu singkat.",
    "rmuAdv.multiFunc": "Multi-Fungsi",
    "rmuAdv.multiFuncDesc": "Menghasilkan produk sampingan bermanfaat (bekatul, sekam).",
    "rmuAdv.reduceLoss": "Kurangi Susut Hasil",
    "rmuAdv.reduceLossDesc": "Meminimalisir kehilangan hasil selama proses giling.",
    "rmuAdv.economic": "Nilai Ekonomi",
    "rmuAdv.economicDesc": "Meningkatkan harga jual beras dengan kualitas premium.",

    // Dryer Advantages Section
    "dryerAdv.badge": "Teknologi Pasca Panen",
    "dryerAdv.title": "Inovasi Pengeringan: Mesin Bed Dryer",
    "dryerAdv.desc": "Solusi pengeringan gabah dan jagung dengan sistem alas datar (bed) untuk menjaga stabilitas kualitas hasil panen di segala cuaca.",
    "dryerAdv.even": "Hasil Merata",
    "dryerAdv.evenDesc": "Kualitas gabah/jagung lebih stabil dan konsisten.",
    "dryerAdv.efficient": "Efisiensi Tinggi",
    "dryerAdv.efficientDesc": "Waktu pengeringan jauh lebih cepat dibanding metode konvensional.",
    "dryerAdv.minLoss": "Minim Susut",
    "dryerAdv.minLossDesc": "Mengurangi kehilangan hasil dan kerusakan butiran.",
    "dryerAdv.energy": "Hemat Energi",
    "dryerAdv.energyDesc": "Optimalisasi penggunaan bahan bakar dan biaya operasional.",
    "dryerAdv.profit": "Untung Meningkat",
    "dryerAdv.profitDesc": "Meningkatkan nilai jual hasil pertanian di pasar.",

    // Dryer Workflow Section
    "dryerWorkflow.title": "Cara Kerja Bed Dryer",
    "dryerWorkflow.desc": "Proses pengeringan modern dengan sistem alas datar bertingkat untuk menjaga kualitas hasil panen Anda.",
    "dryerWorkflow.step1Title": "Pemuatan Bahan",
    "dryerWorkflow.step1Desc": "Gabah/jagung dihamparkan di atas lantai pengering (bed) secara merata.",
    "dryerWorkflow.step2Title": "Hembusan Udara Panas",
    "dryerWorkflow.step2Desc": "Udara panas dari tungku dihembuskan ke atas melalui lantai berlubang secara merata.",
    "dryerWorkflow.step3Title": "Kontrol Suhu & Kelembapan",
    "dryerWorkflow.step3Desc": "Suhu dijaga konstan untuk menurunkan kadar air sesuai standar mutu.",
    "dryerWorkflow.step4Title": "Bahan Kering Keluar",
    "dryerWorkflow.step4Desc": "Hasil kering merata siap untuk proses penyimpanan atau penggilingan.",

    // FAQ Section (Home)
    "faq.title": "Pertanyaan yang Sering Diajukan",
    "faq.subtitle": "Temukan jawaban singkat tentang layanan, ketersediaan suku cadang, dan nilai unggulan kami.",
    "faq.q1": "Mengapa harus membeli mesin penggilingan padi di CV Ateka Tehnik?",
    "faq.a1": "Kami memiliki pengalaman lebih dari 20 tahun dalam manufaktur Rice Milling Unit (RMU) berstandar nasional. Kami memastikan setiap unit dipasang dengan tingkat presisi tinggi untuk meminimalisir persentase beras patah dan meningkatkan rendemen panen Anda.",
    "faq.q2": "Apakah Ateka Tehnik menyediakan layanan purna jual (after-sales) dan suku cadang?",
    "faq.a2": "Ya, kami memberikan garansi fungsi dan ketersediaan suku cadang asli di seluruh wilayah Indonesia. Teknisi kami juga siap dipanggil langsung ke lokasi Anda apabila terdapat kendala operasional, memastikan bisnis penggilingan padi Anda tidak terhenti.",
    "faq.q3": "Bagaimana sistem pemesanan mesin untuk pemerintahan / dinas?",
    "faq.a3": "Pemesanan institusional dapat dilakukan secara transparan melalui E-Katalog LKPP (INAPROC) karena kami adalah vendor resmi. Hubungi kami untuk pembuatan RAB dan desain spesifikasi sesuai anggaran Dinas Pertanian daerah Anda.",
    "faq.q4": "Apakah instalasi mesin sudah termasuk pelatihan operator?",
    "faq.a4": "Tentu, seluruh paket instalasi RMU kami sudah termasuk pelatihan gratis (free training) di lokasi. Kami akan melatih operator atau tim Anda hingga mahir mengoperasikan dan merawat mesin produksi setiap harinya.",
    "faq.readMore": "Baca FAQ Lengkap",

    // FAQ Hub (Dedicated Page)
    "faqHub.badge": "Pusat Informasi Terpadu",
    "faqHub.heroTitle": "Pusat Bantuan & ",
    "faqHub.heroHighlight": "FAQ",
    "faqHub.heroSub": "Solusi terpadu untuk operasi penggilingan padi harian dan aspek teknis alat mesin pertanian.",
    "faqHub.catGeneral": "Umum & Pengiriman",
    "faqHub.qGen1": "Bagaimana metode pengiriman mesin penggilingan padi ke luar pulau Jawa?",
    "faqHub.aGen1": "Pengiriman dilakukan menggunakan armada truk ekspedisi logistik antar-pulau atau kargo kapal laut, mencakup pengiriman ke Sumatera, Kalimantan, Sulawesi, hingga Papua. Tim teknisi ahli kami kemudian akan diberangkatkan menyusul menggunakan peswat atau jalur darat terpisah untuk melakukan perakitan langsung di lokasi Anda.",
    "faqHub.catSpec": "Spesifikasi & Kapasitas",
    "faqHub.qSpec1": "Berapa kapasitas giling mesin RMU ATEKA?",
    "faqHub.aSpec1": "Kami menyediakan paket sistem dengan kapasitas bervariasi: tipe menengah (1-2 ton/jam) dan tipe industri premium (3-5 ton/jam). Untuk skala kecil, tersedia juga unit penggilingan padi keliling (Mobile RMU) dengan mesin diesel 24 PK.",
    "faqHub.qSpec2": "Apa perbedaan sistem statis dan sistem keliling (Mobile RMU)?",
    "faqHub.aSpec2": "Sistem statis dirancang untuk operasi permanen dengan infrastruktur elevator vertikal, sedangkan Mobile RMU menggunakan sasis kendaraan (tipe Colt) untuk layanan jemput bola ke lokasi petani guna menekan biaya logistik gabah.",
    "faqHub.qSpec3": "Apa itu mesin Polisher Satake dan apa keunggulannya?",
    "faqHub.aSpec3": "Polisher Satake adalah komponen premium yang mampu menghasilkan derajat sosoh tinggi dengan tingkat kerusakan butir yang sangat rendah. Mesin ini sangat ideal untuk pengusaha yang menargetkan kualitas beras premium ekspor.",
    "faqHub.catYield": "Kualitas Hasil Giling (Rendemen)",
    "faqHub.qYield1": "Bagaimana cara menekan persentase beras patah (Broken Rice)?",
    "faqHub.aYield1": "Penggunaan konfigurasi mesin yang tepat, seperti pasangan Huller (pecah kulit) dan Polisher (pemutih) dengan pengaturan tekanan yang presisi, dapat menjaga tingkat beras patah di bawah 5-10%.",
    "faqHub.qYield2": "Apakah mesin ATEKA cocok untuk varietas padi lokal seperti Inpari 32 atau Ciherang?",
    "faqHub.aYield2": "Ya, sistem kami telah diuji pada varietas Inpari 32 dan Ciherang yang dominan di Jawa. Dengan pengaturan saringan dan kecepatan giling yang dapat disesuaikan, mesin kami mampu menghasilkan rendemen giling rata-rata di atas 65%.",
    "faqHub.catTechnical": "Teknis & Perawatan",
    "faqHub.qTech1": "Mesin penggerak apa yang digunakan pada sistem penggilingan padi ATEKA?",
    "faqHub.aTech1": "Untuk efisiensi dan torsi tinggi, kami sering menggunakan mesin otomotif yang dimodifikasi (seperti Mitsubishi PS100) atau mesin diesel khusus industri untuk menjamin keawetan operasional jangka panjang.",
    "faqHub.qTech2": "Bagaimana cara menangani limbah sekam dan debu bekatul?",
    "faqHub.aTech2": "Setiap unit RMU ATEKA dapat dilengkapi dengan sistem Blower dan Cyclone (siklon) untuk menangkap debu bekatul dan sekam secara otomatis agar lingkungan kerja tetap bersih dan limbah dapat dikumpulkan untuk pakan ternak.",
    "faqHub.qTech3": "Apakah tersedia mesin pengering (Dryer) untuk memproses gabah di musim hujan?",
    "faqHub.aTech3": "Tersedia unit Bed Dryer dengan kapasitas 8-10 ton yang menggunakan sistem plat frame untuk memastikan gabah mencapai kadar air standar (rata-rata 14%) meskipun tanpa sinar matahari.",
    "faqHub.qTech4": "Bagaimana cara perawatan rutin mesin diesel pada unit RMU berjalan (Selepan Keliling)?",
    "faqHub.aTech4": "Perawatan utama yang wajib dilakukan meliputi penggantian oli secara berkala setiap 200 jam kerja dan pembersihan saringan udara (air filter) setiap minggu. Pembersihan injektor silinder juga wajib dilakukan setiap 3 bulan untuk menjaga pembakaran suhu tinggi tetap efisien, sehingga tenaga mesin konstan atau tidak ngempos saat menggiling volume gabah kering tinggi.",
    "faqHub.qTech5": "Apa perbedaan utama menggunakan mesin penggerak bensin dibandingkan diesel dalam RMU MIni?",
    "faqHub.aTech5": "Mesin diesel industri menghasilkan torsi (torque) mekanis yang jauh lebih kuat di putaran rendah, sehingga sangat adaptif untuk pekerjaan beban berat tanpa henti memutar as utama seperti saat tahapan penggilingan pecah kulit gabah. Sebaliknya, mesin bensin memiliki putaran (RPM) relatif sangat tinggi tapi torsinya statis dan lebih rendah, dan laju konsumsi rasio bahan bakarnya cenderung boros apabila digunakan berjam-jam sehingga menjadi beban bagi operasional harian."
  },

  en: {
    // Navbar
    "nav.home": "Home",
    "nav.products": "Products",
    "nav.about": "About",
    "nav.elkkp": "INAPROC",
    "nav.contact": "Contact",
    "nav.portfolio": "Portfolio",

    // Hero Section
    "hero.badge": "Industrial Leader Since 2004",
    "hero.title": "Rice Mill Supplier: ",
    "hero.titleHighlight": "Precision Solutions",
    "hero.titleSuffix": " for Premium Rice Output",
    "hero.subtitle": "Over 20 years leading the manufacture and installation of nationally standardized Rice Milling Units (RMU). We deliver efficient technology for Indonesia's food security.",
    "hero.cta1": "Business Plan Consultation",
    "hero.cta2": "View INAPROC Catalog",
    "hero.ctaWa": "Contact Us Directly",
    "hero.stat": "Years Experience",

    // TrustBar
    "trustBar.label": "Supported by Strategic Partners & Nationally Certified",

    // ValueProp
    "valueProp.title1": "Expert Technicians",
    "valueProp.desc1": "Every unit is assembled by experienced technicians with strict quality control to ensure operational precision.",
    "valueProp.title2": "Free Operator Training",
    "valueProp.desc2": "We provide intensive training modules so your team can operate machines at maximum efficiency from day one.",
    "valueProp.title3": "Warranty & After-Sales",
    "valueProp.desc3": "Genuine spare parts availability and responsive after-sales service across your entire operational area.",

    // ProductGrid
    "productGrid.title": "Rice Milling Unit Solutions",
    "productGrid.subtitle": "Precision fabrication technology for various scales of rice milling business, from medium capacity to export industry.",
    "productGrid.exploreAll": "Explore All Products",
    "productGrid.detailSpec": "Detail Specifications",
    "productGrid.specLabel": "Unit/Package Specifications:",
    "productGrid.close": "Close",
    "productGrid.consult": "Consult Package",

    // Portfolio (component)
    "portfolio.title": "Our Installations & Projects",
    "portfolio.subtitle": "Over 20 years of delivering precision engineering and reliable Rice Milling Units across Indonesia.",
    "portfolio.viewMore": "View More",

    // ContactMe / Contact Home
    "contact.title": "Contact Us",
    "contact.phone": "Phone / WhatsApp",
    "contact.whatsapp": "WhatsApp",
    "contact.call": "Call",
    "contact.badge": "Ateka Tehnik Hub",
    "contact.subtitle": "Visit our workshop and operational office or contact our representative team for a product consultation.",
    "contact.addressLabel": "Main Workshop Address",
    "contact.addressValue": "Jl. Grompol - Jambangan, Gondang, Kedungjeruk, Kec. Mojogedang, Kabupaten Karanganyar, Central Java",
    "contact.mapsLabel": "Karanganyar Location",

    // OfficialChannelNotice
    "notice.title": "Security Warning",
    "notice.descPre": "The website with the domain ",
    "notice.fakeDomain": "atekateknik.com",
    "notice.descMid": " (using the letter 'k') is ",
    "notice.notOfficial": "NOT our official channel",
    "notice.descPost": ". Any offers or transactions outside our official contacts are not our responsibility.",
    "notice.locationLabel": "The Only Official Location",
    "notice.locationDesc": "Our office & workshop are ONLY located in ",
    "notice.locationCity": "Karanganyar, Central Java",
    "notice.fakeLabel": "Example of Unofficial Channels",
    "notice.fakeDesc": "The website and map address resembling the background behind this are ",
    "notice.fakeDescPost": " owned by Ateka Tehnik. Do not trust if contacted on our behalf from other than our official numbers and addresses.",
    "notice.hoverLabel": "Unofficial Channel Screenshots",
    "notice.ctaBtn": "See Our Official Channels",

    // ChatbotWidget
    "chatbot.name": "Ateka AI Assistant",
    "chatbot.status": "Online & Ready to Help",
    "chatbot.welcome": "Hello! Welcome to CV Ateka Tehnik. How can I help you with your rice milling unit (RMU) manufacturing needs for your business today?",
    "chatbot.popularLabel": "Popular Questions:",
    "chatbot.q1": "What are the specs for the Medium RMU Package?",
    "chatbot.q2": "What is the unit installation procedure?",
    "chatbot.q3": "Do you provide operator training?",
    "chatbot.q4": "What's the estimated cost for the Premium Package?",
    "chatbot.placeholder": "Type your reply...",
    "chatbot.fullscreenExit": "Exit fullscreen",
    "chatbot.fullscreenEnter": "Fullscreen",
    "chatbot.closeChat": "Close chat",
    "chatbot.contactAgent": "Contact Agent",
    "chatbot.techInquiry": "Technical Inquiry",
    "chatbot.disclaimer": "AI responses may not always be accurate",

    // Footer
    "footer.desc": "Pioneer in agricultural processing technology focused on quality and sustainability of Indonesia's agricultural sector.",
    "footer.navigation": "Navigation",
    "footer.home": "Home",
    "footer.products": "Products",
    "footer.projects": "Projects",
    "footer.quickLinks": "Quick Links",
    "footer.privacy": "Privacy Policy",
    "footer.terms": "Terms of Service",
    "footer.faq": "Frequently Asked Questions",
    "footer.globalExport": "Guide & Education",
    "footer.contactOffice": "Contact Office",
    "footer.copyright": "© 2026 CV Ateka Tehnik - All Rights Reserved.",
    "footer.premiumQuality": "Premium Quality",

    // About Page
    "about.badge": "Est. 2004",
    "about.heroTitle1": "Engineering the Future of ",
    "about.heroHighlight": "Rice Milling",
    "about.heroSub": "Two decades of technical mastery in agricultural engineering, dedicated to Indonesian food security and industrial excellence.",
    "about.storyTitle": "A Legacy of Industrial Precision",
    "about.storyP1": "Founded in 2004, CV Ateka Tehnik began with a singular vision: to bridge the gap between traditional agricultural practices and modern industrial efficiency. Over the past ",
    "about.storyP1Bold": "20 years",
    "about.storyP1End": ", we have evolved from a local workshop into a nationally recognized leader in rice milling technology.",
    "about.storyP2Start": "As an ",
    "about.storyP2Bold": "official LKPP vendor",
    "about.storyP2End": ", we are honored to contribute directly to national food security initiatives. Our machines are not just equipment; they are the heart of milling operations across the archipelago, engineered to withstand the rigors of Indonesian climate and throughput requirements.",
    "about.statYears": "Years Experience",
    "about.statInstallations": "Installations",
    "about.statProvinces": "Provinces Served",
    "about.quote": "\"Precision is our baseline. Progress is our mission.\"",
    "about.quoteAuthor": "Warsito - Founding Director",
    "about.coreTitle": "Our Core Directives",
    "about.precisionTitle": "Unrivaled Precision",
    "about.precisionDesc": "We employ aerospace-grade tolerances in our milling assemblies to ensure minimum grain breakage and maximum yield consistency.",
    "about.innovationTitle": "Continuous Innovation",
    "about.innovationDesc": "Our R&D team integrates IoT monitoring and automation into every machine, bringing the Industry 4.0 revolution to the rice fields.",
    "about.partnershipTitle": "Strategic Partnership",
    "about.partnershipDesc": "We don't just sell machines; we build lifelong alliances with millers through dedicated on-site support and maintenance training.",
    "about.leadershipTitle": "Engineering Leadership",
    "about.leadershipSub": "Guided by a team of mechanical specialists and industrial visionaries with decades of on-field expertise.",
    "about.certLabel": "Standards & Compliance",
    "about.certTitle": "Certified Excellence",
    "about.ctaTitle": "Ready to optimize your milling capacity?",
    "about.ctaBtn1": "Discuss Your Project",
    "about.ctaBtn2": "Visit Our Shopee Store",

    // Products Page
    "products.badge": "Precision Engineering",
    "products.heroTitle": "Our Engineering Solutions",
    "products.heroSub": "Experience the convergence of structural stability and agricultural innovation. Each RMU is a masterpiece of precision-machined steel.",
    "products.catAll": "All Products",
    "products.catPaketLengkap": "Complete RMU Package",
    "products.catUnitMesin": "Single Machine Unit",
    "products.catPeralatan": "Supporting Equipment",
    "products.catSukuCadang": "Spare Parts",
    "products.inquire": "Inquire Now",
    "products.buyOnShopee": "Buy on Shopee",
    "products.loading": "Loading products...",
    "products.noProducts": "No products found.",
    "products.downloadSpec": "Download Specs",
    "products.compareTitle": "Package Comparison",
    "products.thFeature": "Technical Feature",
    "products.thStandard": "Standard Pack",
    "products.thBusiness": "Business Pro",
    "products.thPremium": "Premium Enterprise",
    "products.spareParts": "Genuine Spare Parts",
    "products.spareDesc": "Every gear and belt is engineered to the same rigorous standards as our main units. Ensure longevity with Ateka Original.",
    "products.catalogDownload": "Catalog Download",
    "products.visitOfficialStore": "Visit Our Official Store",
    "products.support247": "24/7 Technical Support",
    "products.supportDesc": "Our engineers are ready to assist you anywhere in the archipelago, ensuring minimal downtime for your production.",
    "products.isoCertified": "ISO Certified",
    "products.isoDesc": "Global engineering standards applied locally.",
    "products.globalExport": "Global Export",
    "products.globalDesc": "Serving SEA, Africa, and beyond.",
    "productDetail.engineeringIntro": "For over two decades, Ateka Tehnik has been at the forefront of agricultural engineering in Indonesia. The {product} represents our commitment to bridging the gap between small-scale farming and massive industrial processing.",

    // Contact Page
    "contactPage.heroTitle": "Start Your Industrial Consultation",
    "contactPage.heroSub": "Engineered for precision. Built for scale. Connect with our engineering specialists to optimize your rice milling infrastructure.",
    "contactPage.formTitle": "Technical Inquiry",
    "contactPage.labelName": "Full Name",
    "contactPage.labelCompany": "Company Name",
    "contactPage.labelCapacity": "RMU Capacity (Ton/Hour)",
    "contactPage.selectCapacity": "Select Capacity",
    "contactPage.labelLocation": "Location",
    "contactPage.placeholderLocation": "City, Province",
    "contactPage.labelEmail": "Email Address",
    "contactPage.labelPhone": "Phone Number / WA",
    "contactPage.labelMessage": "Detailed Message",
    "contactPage.messagePlaceholder": "Describe your technical requirements...",
    "contactPage.submitBtn": "Send Inquiry",
    "contactPage.hqLabel": "Headquarters",
    "contactPage.hqTitle": "Central Factory & Office",
    "contactPage.waSupport": "WhatsApp Support",
    "contactPage.emailLabel": "Email Correspondence",
    "contactPage.exportTitle": "Export Capabilities",
    "contactPage.exportDesc": "We offer global shipping and installation services for our milling systems. Our team specializes in cross-border industrial logistics.",
    "contactPage.visitTitle": "Factory Visit",
    "contactPage.visitDesc": "Visits are by appointment only. Please contact our support team at least 48 hours in advance.",
    "contactPage.successTitle": "Inquiry Sent!",
    "contactPage.successDesc": "Thank you, your message has been received. Our team will contact you shortly.",
    "contactPage.errorTitle": "Failed to Send",
    "contactPage.errorDesc": "Sorry, an error occurred while sending your message. Please try again or contact us via WhatsApp.",

    // Portfolio Page
    "portfolioPage.heroTitle": "Our Installations\n& Projects",
    "portfolioPage.heroSub": "Over 20 years of delivering precision-engineered Rice Milling Units across the Indonesian archipelago.",
    "portfolioPage.featuredLabel": "Featured Case Study",
    "portfolioPage.featuredTitle": "RMU Installation 2–3 Ton/Hour",
    "portfolioPage.featuredDesc": "A Rice Milling Unit installation project with a capacity of 2–3 tons per hour in Palembang, South Sumatera. This unit is designed to meet Industrial-scale rice milling needs with high efficiency, producing premium-quality rice ready for distribution.",
    "portfolioPage.featuredCapacityValue": "2–3 Ton/Hour",
    "portfolioPage.statCapacity": "Operational Capacity",
    "portfolioPage.statPurity": "Purity Rating",
    "portfolioPage.viewSpecs": "View technical specs",
    "portfolioPage.gridLabel": "Project Portfolio",
    "portfolioPage.gridTitle": "Engineering Excellence Nationwide",
    "portfolioPage.presenceLabel": "Our Presence",
    "portfolioPage.presenceTitle": "Serving All 34 Provinces",
    "portfolioPage.presenceDesc": "From the western tip of Sumatra to the eastern landscapes of Papua, Ateka Tehnik's engineering footprint spans the entire Indonesian archipelago. Our mobile technician teams ensure that no matter how remote your facility, precision maintenance is always within reach.",
    "portfolioPage.statInstall": "500+ Installations",
    "portfolioPage.statInstallSub": "Operational nationwide",
    "portfolioPage.statSupport": "24/7 Technical Support",
    "portfolioPage.statSupportSub": "Across 3 time zones",
    "portfolioPage.ctaTitle": "Ready to Engineer Your Future?",
    "portfolioPage.ctaDesc": "Whether you're looking to upgrade an existing facility or start a new milling enterprise, our engineers are ready to design your path to precision.",
    "portfolioPage.ctaBtn1": "Start Your Project Consultation",
    "portfolioPage.ctaBtn2": "View Company Profile",

    // Post Page
    "postPage.notFound": "Post Not Found",
    "postPage.notFoundDesc": "The requested article could not be found.",
    "postPage.backToPortfolio": "Back to Portfolio",
    "postPage.noCoverImage": "No Cover Image",
    "postPage.projectOverview": "Project Overview",
    "postPage.share": "Share",
    "postPage.keyDeliverables": "Key Deliverables",
    "postPage.techSpecs": "Technical Specifications",
    "postPage.viewEquipment": "View Equipment",
    "postPage.mediaGallery": "Media Gallery",
    "postPage.comments": "Comments",
    "postPage.addComment": "Add a comment...",
    "postPage.postComment": "Post Comment",
    "postPage.noComments": "No comments yet. Be the first to share your thoughts!",
    "postPage.otherInstallations": "Other Recent Installations",
    "postPage.viewAllProjects": "View all projects",
    "postPage.readCaseStudy": "Read Case Study",
    "postPage.ctaTitle": "Ready to Optimize Your\nOutput?",
    "postPage.ctaDesc": "Ateka Tehnik provides end-to-end engineering solutions for the agricultural industry. From design to commissioning, we deliver precision.",
    "postPage.ctaBtn": "Consult Your Project",
    "postPage.confirmComment": "Confirm Your Comment",
    "postPage.commentReview": "Your comment will be reviewed before appearing publicly.",
    "postPage.postAsAnonymous": "Post as Anonymous",
    "postPage.yourName": "Your Name",
    "postPage.enterName": "Enter your name...",
    "postPage.cancel": "Cancel",
    "postPage.submit": "Submit",
    "postPage.sharePost": "Share this post",
    "postPage.copyLink": "Copy Link",
    "postPage.copied": "Copied!",

    // Official Channels Page
    "channels.badge": "Channel Verification",
    "channels.title": "Official Channels of ",
    "channels.titleHighlight": "Ateka Tehnik",
    "channels.subtitle": "Make sure you only transact and contact us through the communication channels and locations listed below.",
    "channels.officialTitle": "Our OFFICIAL Channels",
    "channels.officialSub": "Valid & Verified (Safe)",
    "channels.officialMapLabel": "Main Factory Location (Karanganyar)",
    "channels.openMaps": "Open in Google Maps",
    "channels.socialLabel": "Official Social Media",
    "channels.fakeTitle": "FAKE / Unofficial Channels",
    "channels.fakeSub": "Please Avoid Transacting Here!",
    "channels.fakeMapLabel": "Scam Location (Tangerang) - NOT US",
    "channels.ignoreLocation": "Ignore this location",
    "channels.viewRef": "View scam reference",
    "channels.fakeWebLabel": "Counterfeit Brand Website",
    "channels.fakeWebNote": "Uses the letter 'k' - NOT US",
    "channels.fakeWebVisit": "Visit",
    "channels.backHome": "Back to Home",

    // Privacy Policy
    "privacy.title": "Privacy Policy",
    "privacy.lastUpdated": "Last updated: April 2, 2026",
    "privacy.intro": "CV Ateka Tehnik (\"us\", \"we\", or \"our\") operates the atekatehnik.com website and associated services (the \"Service\"). This page informs you of our policies regarding the collection, use, and disclosure of personal data when you use our Service and the choices you have associated with that data.",
    "privacy.infoCollectionLabel": "1. Information Collection and Use",
    "privacy.infoCollectionDesc": "We collect several different types of information for various purposes to provide and improve our Service to you. This may include your Name, Email Address, Phone Number (including WhatsApp), Delivery Address (for orders), and Site Usage Data.",
    "privacy.useDataLabel": "2. Use of Data",
    "privacy.useDataDesc": "CV Ateka Tehnik uses the collected data for various purposes: to provide and maintain the Service, notify you about changes, provide customer support and technical consultation (e.g., regarding RMU inquiries), facilitate transactions via LKPP or e-catalog, and monitor the usage of the Service.",
    "privacy.transferDataLabel": "3. Transfer of Data",
    "privacy.transferDataDesc": "Our information systems are based in Indonesia. By using our services, you consent to the processing of data in Indonesia in accordance with local regulations. We will take reasonable steps to ensure that your data is treated securely.",
    "privacy.disclosureLabel": "4. Disclosure of Data",
    "privacy.disclosureDesc": "We may disclose your Personal Data in the good faith belief that such action is necessary to: comply with a legal obligation, protect and defend the rights or property of CV Ateka Tehnik, or prevent/investigate wrongdoing in connection with the Service.",
    "privacy.securityLabel": "5. Security of Data",
    "privacy.securityDesc": "The security of your data is important to us. While no method of transmission over the Internet is 100% secure, we strive to use commercially acceptable means to protect your Personal Data.",
    "privacy.contactLabel": "6. Contact Us",
    "privacy.contactDesc": "If you have any questions about this Privacy Policy, please contact us via WhatsApp or at our headquarters in Dusun Gondang, RT 02 RW 01, Desa Kedungjeruk, Kec. Mojogedang, Karanganyar Regency, Central Java.",

    // Terms of Service
    "terms.title": "Terms of Service",
    "terms.lastUpdated": "Last updated: April 2, 2026",
    "terms.intro": "Please read these Terms of Service (\"Terms\", \"Terms of Service\") carefully before using the atekatehnik.com website (the \"Service\") operated by CV Ateka Tehnik.",
    "terms.acceptanceLabel": "1. Acceptance of Terms",
    "terms.acceptanceDesc": "Your access to and use of the Service is conditioned on your acceptance of and compliance with these Terms. These Terms apply to all visitors, users, and others who access the Service.",
    "terms.servicesLabel": "2. Consultation & Sales Services",
    "terms.servicesDesc": "CV Ateka Tehnik provides manufacturing, installation, and consultation services for Rice Milling Units (RMU). Information on this site summarizes standard specifications; final specifications of each project may be customized and agreed upon via separate contracts or the LKPP E-Catalog process.",
    "terms.intellectualLabel": "3. Intellectual Property",
    "terms.intellectualDesc": "The Service and its original content, features, and functionality (such as UI design, Ateka machine images, technical docs) are and will remain the exclusive property of CV Ateka Tehnik. Such content may not be reproduced without written permission.",
    "terms.linksLabel": "4. Links to Other Web Sites",
    "terms.linksDesc": "Our Service may contain links to third-party web sites that are not owned or controlled by CV Ateka Tehnik (e.g., Indotrading or LKPP platforms). We assume no responsibility for the content or privacy practices of any third-party web sites.",
    "terms.liabilityLabel": "5. Limitation of Liability",
    "terms.liabilityDesc": "In no event shall CV Ateka Tehnik be liable for any indirect, incidental, or consequential damages arising out of accessing or using the materials on this web Service. For physical product warranties such as for RMUs, those are covered under separate Warranty Terms.",
    "terms.governingLabel": "6. Governing Law",
    "terms.governingDesc": "These Terms shall be governed and construed in accordance with the laws of the Republic of Indonesia. Any disputes arising will be resolved under the jurisdiction of the appropriate Indonesian courts.",

    // Edukasi Highlight (Home CTA)
    "edukasi.highlightTitle": "Why Do You Need Modern Machines?",
    "edukasi.highlightDesc": "Discover how modern Rice Milling Unit (RMU) and Bed Dryer machines solve yield loss, grain breakage, and help reduce your operational costs.",
    "edukasi.highlightCta": "Learn More",

    // Edukasi Page
    "edukasi.badge": "Knowledge Center",
    "edukasi.heroTitle": "Post-Harvest ",
    "edukasi.heroHighlight": "Technology Education",
    "edukasi.heroDesc": "Gain deeper insight into modern milling and drying processes to produce super-quality rice and minimize agricultural yield losses.",
    "edukasi.ctaTitle": "Ready to Boost Your Production?",
    "edukasi.ctaDesc": "Take the next step to advance your rice milling business with modern innovation.",
    "edukasi.ctaHome": "Back to Home",
    "edukasi.ctaTech": "Technical Inquiry",
    "edukasi.ctaWa": "Contact via WhatsApp",

    // RMU Workflow Section
    "rmuWorkflow.title": "How the RMU Machine Works",
    "rmuWorkflow.desc": "An integrated automation process to transform raw paddy into premium-quality rice ready for consumption.",
    "rmuWorkflow.step1Title": "Pre-Cleaning",
    "rmuWorkflow.step1Desc": "Cleaning paddy from dirt, stones, and other foreign objects.",
    "rmuWorkflow.step2Title": "Husking",
    "rmuWorkflow.step2Desc": "Separating the husk (chaff) from brown rice grains.",
    "rmuWorkflow.step3Title": "Husk & Rice Separation",
    "rmuWorkflow.step3Desc": "Complete separation of husk, brown rice, and unhusked paddy.",
    "rmuWorkflow.step4Title": "Polishing",
    "rmuWorkflow.step4Desc": "Removing the bran layer to produce shiny white rice.",
    "rmuWorkflow.step5Title": "Grading",
    "rmuWorkflow.step5Desc": "Separating head rice, broken rice, and brewers according to standards.",
    "rmuWorkflow.step6Title": "Packing",
    "rmuWorkflow.step6Desc": "Rice is ready to be packed into sacks with accurate weight.",

    // RMU Advantages Section
    "rmuAdv.title": "RMU Machine Advantages",
    "rmuAdv.desc": "A smart investment for modern rice milling. We deliver significant added value to every grain of rice you produce.",
    "rmuAdv.efficiency": "Process Efficiency",
    "rmuAdv.efficiencyDesc": "Save time, labor, and daily operational costs.",
    "rmuAdv.quality": "Premium Rice Quality",
    "rmuAdv.qualityDesc": "Cleaner, whiter, whole, and shinier rice grains.",
    "rmuAdv.productivity": "High Productivity",
    "rmuAdv.productivityDesc": "Large processing capacity in a short time.",
    "rmuAdv.multiFunc": "Multi-Function",
    "rmuAdv.multiFuncDesc": "Produces valuable by-products (bran, husk).",
    "rmuAdv.reduceLoss": "Reduce Yield Loss",
    "rmuAdv.reduceLossDesc": "Minimize grain loss during the milling process.",
    "rmuAdv.economic": "Economic Value",
    "rmuAdv.economicDesc": "Increase rice selling price with premium quality.",

    // Dryer Advantages Section
    "dryerAdv.badge": "Post-Harvest Technology",
    "dryerAdv.title": "Drying Innovation: Bed Dryer Machine",
    "dryerAdv.desc": "A flat-bed drying solution for paddy and corn to maintain harvest quality stability in all weather conditions.",
    "dryerAdv.even": "Even Results",
    "dryerAdv.evenDesc": "More stable and consistent paddy/corn quality.",
    "dryerAdv.efficient": "High Efficiency",
    "dryerAdv.efficientDesc": "Drying time is much faster than conventional methods.",
    "dryerAdv.minLoss": "Minimal Loss",
    "dryerAdv.minLossDesc": "Reduces yield loss and grain damage.",
    "dryerAdv.energy": "Energy Saving",
    "dryerAdv.energyDesc": "Optimized fuel usage and operational costs.",
    "dryerAdv.profit": "Increased Profit",
    "dryerAdv.profitDesc": "Increases the market value of agricultural products.",

    // Dryer Workflow Section
    "dryerWorkflow.title": "How the Bed Dryer Works",
    "dryerWorkflow.desc": "A modern flat-bed drying process to maintain the quality of your harvest.",
    "dryerWorkflow.step1Title": "Material Loading",
    "dryerWorkflow.step1Desc": "Paddy/corn is spread evenly on the drying bed floor.",
    "dryerWorkflow.step2Title": "Hot Air Blowing",
    "dryerWorkflow.step2Desc": "Hot air from the furnace is blown upward through the perforated floor evenly.",
    "dryerWorkflow.step3Title": "Temperature & Humidity Control",
    "dryerWorkflow.step3Desc": "Temperature is kept constant to reduce moisture content to quality standards.",
    "dryerWorkflow.step4Title": "Dried Material Output",
    "dryerWorkflow.step4Desc": "Evenly dried product ready for storage or milling.",

    // FAQ Section (Home)
    "faq.title": "Frequently Asked Questions",
    "faq.subtitle": "Find quick answers about our services, spare parts availability, and our core value propositions.",
    "faq.q1": "Why should I buy a rice milling machine at CV Ateka Tehnik?",
    "faq.a1": "We have over 20 years of experience in manufacturing national-standard Rice Milling Units (RMU). We ensure every unit is installed with high precision to minimize broken rice percentages and boost your harvest yield.",
    "faq.q2": "Does Ateka Tehnik provide after-sales service and spare parts?",
    "faq.a2": "Yes, we provide functional warranties and genuine spare parts availability across all regions in Indonesia. Our technicians are also ready to be deployed directly to your location if operational issues occur, ensuring your business never stops.",
    "faq.q3": "How does the machine ordering system work for government agencies?",
    "faq.a3": "Institutional orders can be processed transparently through the LKPP e-Catalog (INAPROC) as we are an official verified vendor. Contact us to create budget plans (RAB) and specification designs that align with your regional agriculture department's budget.",
    "faq.q4": "Does the machine installation include operator training?",
    "faq.a4": "Absolutely, all our RMU installation packages include comprehensive free on-site training. We will train your operators or farm team until they are highly proficient in handling and daily maintenance of the production machinery.",
    "faq.readMore": "Read Full FAQ",

    // FAQ Hub (Dedicated Page)
    "faqHub.badge": "Integrated Information Center",
    "faqHub.heroTitle": "Help Center & ",
    "faqHub.heroHighlight": "FAQ",
    "faqHub.heroSub": "Integrated solutions for your daily rice milling operations and technical aspects of agricultural machinery.",
    "faqHub.catGeneral": "General & Shipping",
    "faqHub.qGen1": "How are rice milling machines shipped outside of Java island?",
    "faqHub.aGen1": "Delivery is carried out utilizing inter-island expedition truck fleets or sea freight cargo, covering shipments spanning Sumatra, Kalimantan, Sulawesi, stretching to Papua. Our dedicated team of expert mechanics will subsequently depart using commercial flights or separate terrestrial routes to execute precise on-site assembly immediately.",
    "faqHub.catSpec": "Specifications & Capacity",
    "faqHub.qSpec1": "What is the milling capacity of ATEKA RMU machines?",
    "faqHub.aSpec1": "We provide system packages with varying capacities: medium type (1-2 tons/hour) and premium industrial type (3-5 tons/hour). For small scales, a mobile rice milling unit (Mobile RMU) with a 24 HP diesel engine is also available.",
    "faqHub.qSpec2": "What is the difference between a static system and a mobile system (Mobile RMU)?",
    "faqHub.aSpec2": "Static systems are designed for permanent operation with vertical elevator infrastructure, whereas Mobile RMUs use a vehicle chassis (Colt type) for door-to-door services to farmers' locations to reduce unhusked rice logistics costs.",
    "faqHub.qSpec3": "What is a Satake Polisher machine and what are its advantages?",
    "faqHub.aSpec3": "The Satake Polisher is a premium component capable of producing a high degree of polishing with a very low grain breakage rate. This machine is ideal for entrepreneurs targeting export-quality premium rice.",
    "faqHub.catYield": "Milling Yield Quality",
    "faqHub.qYield1": "How do you minimize the percentage of Broken Rice?",
    "faqHub.aYield1": "Using the correct machine configuration, such as the pairing of a Huller and a Polisher with precise pressure adjustments, can keep the broken rice rate below 5-10%.",
    "faqHub.qYield2": "Are ATEKA machines suitable for local rice varieties like Inpari 32 or Ciherang?",
    "faqHub.aYield2": "Yes, our systems have been tested on Inpari 32 and Ciherang varieties which are dominant in Java. With adjustable sieve and milling speed settings, our machines can produce an average milling yield above 65%.",
    "faqHub.catTechnical": "Technical & Maintenance",
    "faqHub.qTech1": "What drive engines are used in ATEKA rice milling systems?",
    "faqHub.aTech1": "For high efficiency and torque, we frequently use modified automotive engines (like the Mitsubishi PS100) or industry-specific diesel engines to guarantee long-term operational durability.",
    "faqHub.qTech2": "How is rice husk waste and bran dust handled?",
    "faqHub.aTech2": "Every ATEKA RMU unit can be equipped with a Blower and Cyclone system to automatically capture bran dust and husks, keeping the work environment clean and allowing waste to be collected for animal feed.",
    "faqHub.qTech3": "Are there drying machines (Dryers) available for processing unhusked rice in the rainy season?",
    "faqHub.aTech3": "We offer 8-10 ton capacity Bed Dryer units that utilize a flat plate frame system to ensure unhusked rice reaches the standard moisture content (averaging 14%) even without sunlight.",
    "faqHub.qTech4": "What is the routine maintenance for diesel engines on mobile RMU units?",
    "faqHub.aTech4": "Mandatory primary maintenance includes periodic oil substitution every 200 operational hours and weekly air filter purification. Cylinder injector cleansing is also strictly required every quarter to maintain high-temperature combustion efficiency, guaranteeing constant mechanical torque without stalling while milling high volumes of dried unhusked rice.",
    "faqHub.qTech5": "What is the primary difference between using a gasoline vs. a diesel engine driver?",
    "faqHub.aTech5": "Industrial diesel engines generate drastically superior mechanical torque (turning force) at low revolutions, rendering them highly adaptive for continuous heavy-duty loading applications turning mainshafts, such as the initial hulling stage. Conversely, gasoline engines possess relatively very high revolutions (RPMs) but lower static torque, and their fuel-to-power consumption ratio tends to be vastly uneconomical for extensive daily operational burdens."
  }
};

export default translations;
